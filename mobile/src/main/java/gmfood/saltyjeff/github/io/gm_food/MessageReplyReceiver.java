...
public class MessageReplyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (MessageService.REPLY_ACTION.equals(intent.getAction())) {
            int conversationId = intent.getIntExtra(MessageService.CONVERSATION_ID, -1);
    ...
    private CharSequence getMessageText(Intent intent) {
        ...
        if (remoteInput != null) {
            return remoteInput.getCharSequence(MessageService.EXTRA_VOICE_REPLY);
        }
        return null;
    }
}
